...
// Usa VERCEL_URL / API_BASE_URL si existen; si no, localhost
const API_BASE_URL =
...
async function obtenerProductosCafeteria() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/productos?categoria=cafeteria`
    );
    const data = await response.json();
    ...
    data.forEach((producto) => {
      console.log(
        `  - ${producto.nombre} - $${producto.precio}${
          producto.descuento > 0 ? ` (${producto.descuento}% desc.)` : ""
        }`
      );
    });
  } catch (error) {
...
async function obtenerProductosLibreria() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/productos?categoria=libreria`
    );
    const data = await response.json();
...
async function obtenerProductosDisponibles(disponible) {
  ...
  console.log(
    `\n✨ PETICIÓN 7: Obtener productos ${
      disponible === "false" ? "NO disponibles" : "disponibles"
    }`
  );
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/productos/disponibles${queryParam}`
    );
    const data = await response.json();
...
async function obtenerProductosNoDisponibles() {
  ...
  console.log(
    "URL:",
    `${API_BASE_URL}/api/productos/disponibles?disponible=false`
  );
  console.log("-".repeat(60));
